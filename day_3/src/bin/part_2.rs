...
fn get_gear_positions(input: &str) -> Vec<Position> {
    let mut positions: Vec<Position> = vec![];
    for (row, line) in input.lines().enumerate() {
        for (col, char) in line.chars().enumerate() {
            if char == '*' {
...
fn get_gears(input: &str, part_numbers: &Vec<PartNumber>) -> Vec<Gear> {
    let gear_positions = get_gear_positions(input);

    let gears: Vec<Gear> = gear_positions.iter().map(|position| Gear { value: 0, position: *position }).collect();


    let gears = gears.iter().map(|gear| {
        let mut surrounding_numbers: Vec<isize> = vec![];
        for part_number in part_numbers {
            for position in part_number.positions.iter() {
                if position == &gear.position {
                    surrounding_numbers.push(part_number.number)
                }
            }
        }
        
        let value = match surrounding_numbers.len() {
            2 => surrounding_numbers.iter().fold(1, |acc, n| { acc * n }),
            _ => 0
        };

        Gear { value, position: gear.position }
    }).collect();
...
fn part_2(input: &str) -> isize {
    ...
    let gears = get_gears(input, &part_numbers);
